...
std::vector<cv::DMatch> descriptor_matcher(const cv::Mat& descriptors1, const cv::Mat& descriptors2, const float tresh) {
    ...
    for (auto& m : knnMatches) {
        if (m[0].distance < tresh*m[1].distance) { //can change this value to say how much better the best match has to be compared to second best
            good.push_back(m[0]);
        }}
    return good;
}
